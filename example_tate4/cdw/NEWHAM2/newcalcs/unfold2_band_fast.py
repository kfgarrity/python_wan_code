...
coords2, types2 = ops.parse_coords(coords_small)
...
orbital_info_big = [["Ta", 2, ["d"]], ["Te", 8, ["p"]]]
...
h = wan_ham('../../small_nod/qe_hr.dat') #load the hamiltonian
...
image, limits, imageproj = ops.unfold_bandstructure(h_small, h_super_r,supercell,kpts, yrange = [-3, 3], names = names, temp=0.02, fermi=fermi, pdfname="unfold_cdw_TEST.pdf", num_energies=300, PROJARR=ORBS)
ops.plot_image(image, limits, names=names, pdfname="unfold_cdw_image.pdf")
np.savetxt("unfold_cdw_image_TEST.csv", image)
np.savetxt("unfold_cdw_image_proj0_TEST.csv", imageproj[:,:,0])
np.savetxt("unfold_cdw_image_proj1_TEST.csv", imageproj[:,:,1])
np.savetxt("unfold_cdw_image_proj2_TEST.csv", imageproj[:,:,2])
np.savetxt("unfold_cdw_image_proj3_TEST.csv", imageproj[:,:,3])

plt.clf()

ops.plot_image(imageproj[:,:,0], limits, names=names, pdfname="unfold_cdw_image_proj0_TEST.pdf")
plt.clf()
ops.plot_image(imageproj[:,:,1], limits, names=names, pdfname="unfold_cdw_image_proj1_TEST.pdf")
plt.clf()
ops.plot_image(imageproj[:,:,2], limits, names=names, pdfname="unfold_cdw_image_proj2_TEST.pdf")
plt.clf()
ops.plot_image(imageproj[:,:,3], limits, names=names, pdfname="unfold_cdw_image_proj3_TEST.pdf")


exit()
    
#for fermi_adj in [0.0, -0.05, -0.1, -0.15]:
for fermi_adj in [-0.1, 0.0, 0.1]:
